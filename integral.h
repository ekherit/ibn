...
L160:
  ...
  //Многомерный интеграл
  template <int n /*dimention*/ , class F> 
    double dgausD(const F & func, double *a, double *b, double eps, double &relerr)
  {
    double ctr[15], wth[15], wthl[15], z[15];
    //int n = func.dimension();
    const double xl2 = 0.358568582800318073;
    ...
    double abserr = 0;
    int ifail = 3;
    static_assert( 2<= n && n<=15);

    constexpr int twondm = std::pow(2,n);
    int ifncls = 0;
    bool ldv   = false;
    constexpr int irgnst = 2*n+3;
    constexpr int irlcls = twondm +2*n*(n+1)+1;
    int isbrgn = irgnst;
    int isbrgs = irgnst;
    ...
    //   where MAXPTS = Maximum number of function evaluations to be allowed.
    //   Here we set MAXPTS to 1000*(the lowest possible value)
    constexpr int maxpts = 1000*irlcls;
    constexpr int minpts = 1;

    // The original agorithm expected a working space array WK of length IWK
    // with IWK Length ( >= (2N + 3) * (1 + MAXPTS/(2**N + 2N(N + 1) + 1))/2).
    // Here, this array is allocated dynamically

    constexpr int iwk = irgnst*(1 +maxpts/irlcls)/2;
    //double *wk = new double[iwk+10];
    std::array<double,iwk+10> wk;
    int j;
    for (j=0;j<n;++j) {
      ctr[j] = (b[j] + a[j])*0.5;
      wth[j] = (b[j] - a[j])*0.5;
